...
Examples:
  ...
  python -m src.cli_updated --question "..."
    ...
    parser.add_argument("--with-critic", "-c", action="store_true", help="Enable critic feedback")
    parser.add_argument("--virtual-lab", "-vl", action="store_true", help="Original Virtual Lab (parallel)")
    parser.add_argument("--subtask-centric", "-sc", action="store_true", help="NEW: Subtask-Centric Virtual Lab (sequential, recommended)")
    parser.add_argument("--compare-vl", action="store_true", help="Compare old vs new Virtual Lab side-by-side")
    parser.add_argument("--combined", action="store_true", help="Combined Mode (LangGraph + Consensus)")
    parser.add_argument("--langgraph", action="store_true", help="LangGraph workflow")
    parser.add_argument("--rounds", "-r", type=int, default=2, help="Number of rounds (default: 2 for old VL, 1 for subtask-centric)")
    parser.add_argument("--team-size", "-t", type=int, default=3, help="Max team size (default: 3)")
    parser.add_argument("--api-key", type=str, help="API key")
    parser.add_argument("--data-dir", "-d", type=str,
                       default=os.getenv("DATABASE_DIR", "/home.galaxy4/sumin/project/aisci/Competition_Data"),
                       help="Path to database directory")
    parser.add_argument("--input-dir", type=str, default=os.getenv("INPUT_DIR"),
    ...
    if args.subtask_centric:
        ...
        print(f"Configuration: {args.rounds} round(s), max {args.team_size} specialists")
        ...
        output_file = save_answer_to_file(final_answer, args.question, args.output, mode="subtask-centric-vl")
    ...
    if args.compare_vl:
        ...
        # Save both
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        old_file = save_answer_to_file(old_answer, args.question,
                                       f"comparison_old_{timestamp}.md", "original-vl")
        new_file = save_answer_to_file(new_answer, args.question,
                                       f"comparison_new_{timestamp}.md", "subtask-centric-vl")
