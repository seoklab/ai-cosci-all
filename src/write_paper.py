...
def print_validation_report(validation: dict):
    ...
    print(f"🔗 Citations: {validation.get('citation_count', 0)}")
    print(f"📊 Figures: {validation.get('figure_suggestions', 0)}")
    ...
    print(f"\n🏗️  Structure Complete: {'✅ Yes' if validation.get('structure_complete') else '❌ No'}")
    print(f"📚 References Quality: {'✅ Good' if validation.get('references_populated') else '❌ Needs Work'}")
...
Examples:
  ...
  python src/write_paper_re.py --question "Research question?" --results "Findings..." --verbose
    ...
    # Print header
    print("🚀 REFACTORED ACADEMIC PAPER GENERATOR")
    print("=" * 70)
    
    try:
        # Load research question
        if args.question_file:
            print(f"\n📖 Loading research question from: {args.question_file}")
            research_question = load_content_from_file(args.question_file)
        else:
            research_question = args.question
            print(f"📖 Using direct research question ({len(research_question)} chars)")
        
        # Load scientist results
        if args.results_file:
            print(f"📊 Loading scientist results from: {args.results_file}")
            scientist_findings = load_content_from_file(args.results_file)
        else:
            scientist_findings = args.results
            print(f"📊 Using direct scientist results ({len(scientist_findings)} chars)")
        
        # Initialize paper solver
        print("\n🤖 Initializing PaperSolver...")
        ...
        # Generate paper
        print(f"\n📝 Generating paper (target: {args.words} words)...")
        ...
        try:
            ...
            print(f"\n❌ Paper generation failed: {paper_error}")
            if args.verbose:
                import traceback
                traceback.print_exc()
            ...
            print("\n❌ Cannot continue due to paper generation failure")
            sys.exit(1)
        
        # Save paper with metadata
        print(f"\n💾 Saving paper with metadata to: {args.output}")
        ...
        print("\n🎉 SUCCESS!")
        print("=" * 60)
        print(f"📄 Paper Generated: {saved_path}")
        print(f"📏 Length: {validation.get('word_count', 0)} words")
        print(f"🔗 Citations: {validation.get('citation_count', 0)}")
        print(f"📊 Structure: {'Complete' if validation.get('structure_complete') else 'Incomplete'}")
        ...
        if (validation.get('structure_complete') and 
            ...
            print("\n🔧 PAPER QUALITY: Good structure, enhance citations")
        else:
            print("\n⚠️  PAPER QUALITY: Review and improve structure")
