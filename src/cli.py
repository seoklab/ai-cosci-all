...
def save_answer_to_file(answer: str, question: str, output_path: str = None, mode: str = "single", evaluation = None) -> str:
    ...
    Args:
        ...
        evaluation: Optional EvaluationResult object from FastChat critic
...
**Judge Model:** {evaluation.judge_model}
...
"""
        if evaluation.scores:
            for category, score in sorted(evaluation.scores.items(), key=lambda x: x[1].score, reverse=True):
                content += f"**{category.replace('_', ' ').title()}:** {score.score:.1f}/10\n"
                if score.reasoning:
                    content += f"  - *Reasoning:* {score.reasoning}\n"
                if score.strengths:
                    content += f"  - *Strengths:* {', '.join(score.strengths)}\n"
                if score.weaknesses:
                    content += f"  - *Weaknesses:* {', '.join(score.weaknesses)}\n"
                if score.suggestions:
                    content += f"  - *Suggestions:* {', '.join(score.suggestions)}\n"
                content += "\n"
        
        content += "\n---\n\n"

    content += "*Generated by CoScientist - AI Research Assistant*\n"
  ...
  python -m src.cli --question "..." --verbose
    ...
    if args.question:
        ...
        if args.combined:
            ...
            if args.evaluate:
                print("\n" + "=" * 60)
                print("FASTCHAT EVALUATION")
                print("=" * 60)
                try:
                    from src.agent.fastchat_critic import FastChatCritic
                    ...
                    evaluation = critic.evaluate(
                        ...
                        include_all_categories=args.eval_categories is None,
                    )
            ...
            output_file = save_answer_to_file(final_answer, args.question, args.output, mode="combined", evaluation=evaluation)
            ...
            if args.evaluate:
                ...
                try:
                    ...
                    evaluation = None  # No evaluation on error
                    print("\n" + "=" * 60)
                    print("Final Answer:")
                    print("=" * 60)
                    print(response)
            ...
            # Save to file (include evaluation if available)
            output_file = save_answer_to_file(response, args.question, args.output, mode="single-agent", evaluation=evaluation)
            print(f"\n✓ Answer saved to: {output_file}")
