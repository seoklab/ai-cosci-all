...
def save_answer_to_file(answer: str, question: str, output_path: str = None, mode: str = "single", 
                       eval_score: float = None, eval_explanation: str = None) -> str:
...
def auto_evaluate_answer(answer: str, question: str, verbose: bool = False):
    ...
    try:
        ...
        print("\n" + "="*60)
        print(f"📊 FINAL SCORE: {eval_result.score:.1f}/10.0")
        print("="*60)
        
        if not verbose:
            # Show brief explanation if not in verbose mode
            explanation_preview = eval_result.explanation[:300] + "..." if len(eval_result.explanation) > 300 else eval_result.explanation
            print(f"\n{explanation_preview}")
        
        return eval_result.score, eval_result.explanation
        
    except Exception as e:
        print(f"\n⚠️  Auto-evaluation failed: {e}")
  ...
  python -m src.cli --question "..." --verbose
    ...
    if args.question:
        ...
        if args.combined:
            ...
            # Auto-evaluate
            eval_score, eval_explanation = auto_evaluate_answer(final_answer, args.question, args.verbose)
            
            # Save answer with evaluation
            output_file = save_answer_to_file(
                final_answer, args.question, args.output, mode="combined",
                eval_score=eval_score, eval_explanation=eval_explanation
            )
            print(f"\n✓ Answer with evaluation saved to: {output_file}")
            ...
            eval_score, eval_explanation = auto_evaluate_answer(final, args.question, args.verbose)
            ...
            # Auto-evaluate
            eval_score, eval_explanation = auto_evaluate_answer(response, args.question, args.verbose)
