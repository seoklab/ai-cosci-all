...
Synthesize across all {num_rounds} rounds to provide the most complete and detailed answer possible."""
        ...
        final_answer_internal = self.pi.run(final_prompt, verbose=self.verbose)

        self.meeting_transcript.append({
            "speaker": "PI",
            "role": "Final Synthesis (Internal)",
            "content": final_answer_internal
        })
        ...
        if critical_flags:
            addressed_count = sum(
                ...
                if flag['flag_id'] in final_answer_internal or flag['issue'][:30] in final_answer_internal
            )
            ...
            if addressed_count < len(critical_flags):
                final_answer_internal += f"\n\n⚠️ **WARNING:** Not all critical red flags were fully addressed ({addressed_count}/{len(critical_flags)}). Further iteration may be needed."

        # Save internal version with red flag resolution as intermediate file
        self._save_internal_final_answer(final_answer_internal, num_rounds)

        # Generate clean submission version (without meta-commentary about critique satisfaction)
        self.logger.progress("Generating clean submission answer...", indent=2)

        submission_prompt = f"""Create a comprehensive final submission answer based on the research findings.

**Original Question:** "{self.user_question}"

**Research Findings (Internal Synthesis):**
...
The answer should stand alone as a complete response without referencing the internal review process."""
        ...
        self.meeting_transcript.append({
            "speaker": "PI",
            "role": "Final Submission Answer",
            "content": final_answer_submission
        })

        # Append references
        self.logger.progress("Appending references section...", indent=2)
        final_answer_with_refs = self._append_references_section(final_answer_submission)
...
"""
    ...
    def _save_internal_final_answer(self, internal_answer: str, num_rounds: int):
        """Save internal final answer with red flag resolution for record-keeping.

        Args:
            internal_answer: The internal synthesis including red flag resolution
            num_rounds: Total number of rounds completed
        """
        if not self.save_intermediate:
            return
        ...
        filename = "final_answer_internal.md"
        filepath = run_dir / filename

        # Format content
        content = f"""# Internal Final Answer (With Red Flag Resolution)
...
This version includes the red flag resolution and quality assurance process.
This is for internal record-keeping and quality validation.
...
*Generated by CoScientist Subtask-Centric Virtual Lab*
*This is the internal version - see the main answer file for the clean submission version*
"""

        with open(filepath, 'w', encoding='utf-8') as f:
            f.write(content)

        self.logger.verbose(f"Saved internal final answer to: {filepath.name}", indent=2)
